...
class ConfidenceAnalysis:
    ...
    def __init__(self, uq_data, copula, num_intervals=10, custom_data_point=None):
        ...
        for var in self.copula.input_names:
            self.calculate_variable_probabilities(var, self.num_intervals)
            self.perform_optimization(var)
            # Example: Change the constraint to allow any number of intervals from 1 to 5
        #        4. Modify data for plotting
        self.plotting_data = self.modify_data(self.variable_data)

    def interval_constraint(self, x, num_intervals):

        return np.sum(x) - num_intervals

    def perform_optimization(self, var):
        initial_selection = np.ones(self.num_intervals, dtype=int)
        num_intervals_to_optimize = 5
        while num_intervals_to_optimize > 2:
            # Initialize the initial selection based on the desired number of intervals
            initial_selection = np.ones(num_intervals_to_optimize, dtype=int)

            # Define constraints for the optimization
            # Define constraints for the optimization
            constraints = {
                "type": "eq",
                "fun": lambda x, num_intervals=num_intervals_to_optimize: np.sum(x)
                - num_intervals,
            }
            ...
            result = minimize(
                ...
                initial_selection,
                args=(var, self.num_intervals),
                constraints=constraints,
                bounds=[(0, 1)] * num_intervals_to_optimize,
                method="SLSQP",
            )
            ...
            print(var, result)
            num_intervals_to_optimize -= 1

            # initial_selection = np.zeros(self.num_intervals)
            # initial_selection[:5] = 1  # Select the first 2 intervals
            # num_intervals_to_optimize = (
            #     self.num_intervals
            # )  # Adjust the number of intervals as needed
            # constraints = (
            #     {
            #         "type": "eq",
            #         "fun": lambda x, num_intervals=num_intervals_to_optimize: self.interval_constraint(
            #             x, num_intervals
            #         ),
            #     },
            # )
            ...
            #     self.objective_function,
            #     initial_selection,
            #     args=(var, num_intervals_to_optimize),
            #     constraints=constraints,
            #     bounds=[(0, 1)] * self.num_intervals,
            #     method="SLSQP",
            # )
            # print(var, result.x)

    def objective_function(self, x, var, num_intervals):
        ...
        selected_confidences = variable_data.interval_confidence.values[
            selected_indices
        ]
        selected_errors = variable_data.interval_confidence_uncertainty.values[
            selected_indices
        ]
        ...
        overlap_area = self.overlap(selected_confidences, selected_errors)
